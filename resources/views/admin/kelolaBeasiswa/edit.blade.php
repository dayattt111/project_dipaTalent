...
<div class="max-w-3xl">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-600 mt-1">Ubah informasi beasiswa</p>
    </div>

    {{-- Form Card --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.kelolaBeasiswa.update', $beasiswa->id) }}" method="POST" class="space-y-6">
            @csrf

            {{-- Nama Beasiswa --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Nama Beasiswa</label>
                <input type="text" name="nama_beasiswa" value="{{ old('nama_beasiswa', $beasiswa->nama_beasiswa) }}" placeholder="Masukkan nama beasiswa" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4" placeholder="Masukkan deskripsi beasiswa" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ old('deskripsi', $beasiswa->deskripsi) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="kuota" value="{{ old('kuota', $beasiswa->kuota) }}" placeholder="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    @error('kuota') <div class="text-red-600 text-sm mt-1">{{ $message }}</div> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Mulai</label>
                    <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', is_string($beasiswa->tanggal_mulai) ? $beasiswa->tanggal_mulai : $beasiswa->tanggal_mulai?->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    @error('tanggal_mulai') <div class="text-red-600 text-sm mt-1">{{ $message }}</div> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Selesai</label>
                    <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', is_string($beasiswa->tanggal_selesai) ? $beasiswa->tanggal_selesai : $beasiswa->tanggal_selesai?->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
